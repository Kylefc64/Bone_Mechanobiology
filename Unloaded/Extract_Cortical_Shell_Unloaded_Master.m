function Extract_Cortical_Shell_Unloaded_Master(spec_nums, num_slices)

%%%%%%%%%%% Enter inputs below %%%%%%%%%%
num_specs = length(spec_nums);

for i = 1:num_specs
    fprintf(['Creating UV cortical shells for specimen R', spec_nums{i}, ' ...\n']);
    
    innerMaskDir = ['\\Biomech-10\j\RTL06_R', spec_nums{i}, '_C8_Processed\Tiled\Masks\'];
    innerMaskName = ['RTL06_R', spec_nums{i}, '_C8_masks_'];
    
    outerMaskDir = ['\\Biomech-10\i\RTL06_Cortical_Processed\RTL06_R', spec_nums{i}, '_C8_Processed\CT_binary_registered\'];
    outerMaskName = 'CT_Binary_Registered_';
    
    imageDir = ['\\Biomech-10\j\RTL06_R', spec_nums{i}, '_C8_Processed\Tiled\Gray\OP\'];
    imageName = ['RTL06_R', spec_nums{i}, '_C8_gray'];
    
    targetDir = ['\\Biomech-10\i\RTL06_Cortical_Processed\RTL06_R', spec_nums{i}, '_C8_Processed\UV_Cortical_Shells\'];
    targetName = ['RTL06_R', spec_nums{i}, '_C8_UV_Cortical_Shell_'];
    
    beginIndex = 0;
    endIndex = num_slices(i) - 1;
    ...
    while(sliceIndex <= endIndex)
        if mod(sliceIndex, 10) == 0
           fprintf(['Creating UV cortical shell for slice ', num2str(sliceIndex, '%04d'), ' ...\n']); 
        end
