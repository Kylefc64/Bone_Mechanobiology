function PostLoad_BF_method3_cortical(spec_num)
...
for loop = 1:4
    ...
    %%% NEW %%%
    %num_fitc_objs = max(max(max(FITC_labeled)));
    di_oxy_labeled = bwlabeln(Oxy_di);
    %num_di_oxy_objs = max(max(max(di_oxy_labeled)));
    %overlap = Oxy_di .* fitc_close;
    FITC_labeled_filtered = zeros(size(FITC_labeled));
    di_oxy_labeled_filtered = zeros(size(di_oxy_labeled));
    both = uint16(di_oxy_labeled) .* uint16(FITC_labeled);
    %fitc_keep = unique(both ./ uint16(di_oxy_labeled));
    fitc_keep = unique(Oxy_di .* FITC_labeled);
    %di_oxy_keep = unique(both ./ uint16(FITC_labeled));
    di_oxy_keep = unique(fitc_close .* di_oxy_labeled);
    both = unique(both);
    %relabel
    for i = 1:length(fitc_keep)
        FITC_labeled_filtered(find(FITC_labeled == fitc_keep(i))) = i;
    end
    for i = 1:length(di_oxy_keep)
        di_oxy_labeled_filtered(find(di_oxy_labeled == di_oxy_keep(i))) = i;
    end
    
    %FITC_labeled_filtered = bwlabeln(FITC_labeled_filtered,26);
    %di_oxy_labeled_filtered = bwlabeln(di_oxy_labeled_filtered,26);
    num_fitc_objs = max(max(max(FITC_labeled_filtered)));
    num_di_oxy_objs = max(max(max(di_oxy_labeled_filtered)));
    
    tic
    for i = 1:num_fitc_objs
        display(i)
        for j = 1:num_di_oxy_objs
            display(j)
            fitc_obj_img = zeros(size(fitc_close));
            di_oxy_obj_img = zeros(size(oxy_close));
            fitc_obj_img(find(FITC_labeled == i)) = 1;
            di_oxy_obj_img(find(di_oxy_labeled == j)) = 1;
            %if di_oxy_obj_img and fitc_obj_img overlap
            if max(max(max(di_oxy_obj_img .* fitc_obj_img))) > 0
                size_fitc = sum(sum(sum(fitc_obj_img)));
                size_di_oxy = sum(sum(sum(di_oxy_obj_img)));
                if size_di_oxy < 0.5*size_fitc
                    fprintf('fitc object too large...\n');
                    fitc_close = fitc_close - (fitc_obj_img .* imdilate(di_oxy_obj_img, SE_3D));
                    %fitc_close(find(fitc_obj_img .* imdilate(oxy_obj_img, SE_3D) > 0)) = 0;
                else
                    fprintf('whole fitc object removed...\n');
                    fitc_close = fitc_close - fitc_obj_img;
                    %fitc_close(find(FITC_labeled == i)) = 0;
                end
            end
        end
    end
    ...
    %di_oxy_labeled = bwlabeln(Oxy_di);
    %both = di_oxy_labeled .* FITC_labeled;
    %FITC_labeled_overlap = both ./ di_oxy_labeled;
    %FITC_labeled_overlap(isnan(FITC_labeled_overlap)) = 0;
    %di_oxy_labeled_overlap = both ./ FITC_labeled;
    %di_oxy_labeled_overlap(isnan(di_oxy_labeled_overlap)) = 0;
    %both = unique(both);
    ...
    for i = 1:length(both)
        display(i)
        fitc_inds = find(mod(both(i), fitc_keep) == 0);
        display(length(fitc_inds))
        for j = 1:length(fitc_inds)
            display(j)
            di_oxy_inds = find(mod(both(i), fitc_inds(j)) == di_oxy_keep);
            display(length(di_oxy_inds))
            if length(di_oxy_inds ~= 0)
                size_fitc = size(find(FITC_labeled == fitc_inds(j)), 1);
                fitc_obj_img = zeros(size(fitc_close));
                fitc_obj_img(find(FITC_labeled == fitc_inds(j))) = 1;
            end
            for k = 1:length(di_oxy_inds)
                display(k)
                size_di_oxy = size(find(di_oxy_labeled == di_oxy_inds(k)), 1);
                if size_di_oxy < 0.5*size_fitc
                    oxy_obj_img = zeros(size(oxy_close));
                    oxy_obj_img(find(di_oxy_labeled == di_oxy_inds(j))) = 1;
                    fitc_close = fitc_close - (fitc_obj_img .* imdilate(oxy_obj_img, SE_3D));
                else
                    fitc_close = fitc_close - fitc_obj_img;
                end
            end
    ...
    %{
        %fitc_keep = unique(overlap .* FITC_labeled);
        %FITC_labeled_filtered(FITC_labeled_filtered ~= all(fitc_keep)) = 0;
        %di_oxy_keep = unique(overlap .* di_oxy_labeled);
        %di_oxy_labeled_filtered(di_oxy_labeled_filtered ~= all(di_oxy_keep)) = 0;
        for i = 1:num_fitc_objs
            display(i)
            fitc_obj_img = zeros(size(fitc_close));
            fitc_obj_img(find(FITC_labeled == i)) = 1;
            if max(max(max(overlap .* fitc_obj_img))) == 0
                FITC_labeled_filtered = FITC_labeled_filtered - fitc_obj_img;
            end
        end
        num_filtered_fitc_objs = size(unique(FITC_labeled_filtered), 1);
        for j = 1:num_di_oxy_objs
            display(j)
            oxy_obj_img = zeros(size(oxy_close));
            oxy_obj_img(find(di_oxy_labeled_filtered == j)) = 1;
            if max(max(max(overlap .* oxy_obj_img))) == 0
                di_oxy_labeled_filtered = di_oxy_labeled_filtered - oxy_obj_img;
            end
        end
        num_filtered_di_oxy_objs = size(unique(di_oxy_labeled_filtered), 1);
    %}
    ...
    for i = 1:num_fitc_objs
        for j = 1:num_di_oxy_objs
            display(i)
            display(j)
            %if di_oxy and fitc overlap
            fitc_obj_img = zeros(size(fitc_close));
            oxy_obj_img = zeros(size(oxy_close));
            fitc_obj_img(find(FITC_labeled == i)) = 1;
            oxy_obj_img(find(di_oxy_labeled == j)) = 1;
            %if max(max(max(oxy_close(find(di_oxy_labeled == j)) .* fitc_close(find(FITC_labeled == i))))) > 0
            if max(max(max(oxy_obj_img .* fitc_obj_img))) > 0
                size_di_oxy = size(find(di_oxy_labeled == j), 1);
                size_fitc = size(find(FITC_labeled == i), 1);
                if size_di_oxy < 0.5*size_fitc
                    fitc_close = fitc_close - (fitc_obj_img .* imdilate(oxy_obj_img, SE_3D));
                    %fitc_close(find(fitc_obj_img .* imdilate(oxy_obj_img, SE_3D) > 0)) = 0;
                else
